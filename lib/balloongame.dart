...
//variables that do not change with game state
...
class _BalloonGameState extends State<balloongame> with TickerProviderStateMixin {
  ...
  void insMathGameInfo(String gname,int m,var s, var e,String cid) async {
    ...
    String plma = json.encode(playmath);

    final response = await http.post(
        'https://braintrainapi.com/btapi/player_maths',
        headers: {"Content-Type": "application/json"}, body: plma);
  ...
  //dispose of animation and sounds when done
  ...
  dispose() {
    //dispose of animation and call super.dispose()
    sparklesAnimationController.dispose();
    super.dispose();

    //Insert info to DB for game session
    var end = DateTime.now();
    insMathGameInfo(gameName,match,start,end,cid);

    //sound and sound subscriptions.
    soundManager.stop();
    soundStream.cancel();
  }
  ...
  Widget build(BuildContext context) {
    ...
    {
      soundStream = Future.delayed(const Duration(milliseconds: 500), () {
    ...
    //values that move the balloons to the correct positions
          ...
          new Positioned(
            ...
            child: FlatButton(
              ...
              onPressed: () {
                ...
                if(num2 == 0)
                {
                  //increment match for game session correct answers
                  match++;
                  ...
                  soundStream = Future.delayed(const Duration(milliseconds: 4500), () {
                  }).asStream().listen((Null) {
